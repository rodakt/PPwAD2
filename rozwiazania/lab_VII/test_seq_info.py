...
class TestIsArithmetic(unittest.TestCase):
    """Testy funkcji is_arithmetic() bez podanej różnicy"""
...
class TestIsArithmeticWithR(unittest.TestCase):
    """Testy funkcji is_arithmetic() z podaną różnicą r"""

    def test_empty_with_r(self):
        """Test dla pustej sekwencji z podaną różnicą"""
        self.assertTrue(is_arithmetic([], r=5))

    def test_single_with_r(self):
        """Test dla jednoelementowej sekwencji z podaną różnicą"""
        self.assertTrue(is_arithmetic([1], r=3))

    def test_positive(self):
        """Test dla sekwencji arytmetycznych z prawidłową różnicą"""
        self.assertTrue(is_arithmetic([1, 4, 7, 10], r=3))
        self.assertTrue(is_arithmetic(range(0, 100, 7), r=7))
        self.assertTrue(is_arithmetic(range(100, 0, -4), r=-4))
        self.assertTrue(is_arithmetic([5, 5, 5], r=0))

    def test_negative(self):
        """Test dla sekwencji arytmetycznych z nieprawidłową różnicą"""
        self.assertFalse(is_arithmetic([1, 4, 7, 10], r=2))
        self.assertFalse(is_arithmetic([1, 3, 5], r=3))
        self.assertFalse(is_arithmetic([5, 5, 5], r=1))

    def test_non_arithmetic_with_r(self):
        """Test dla sekwencji niearytmetycznych z podaną różnicą"""
        for seq in NON_ARITHMETIC_SEQS:
            with self.subTest(seq=seq):
                self.assertFalse(is_arithmetic(seq, r=1))


class TestIsGeometric(unittest.TestCase):
    """Testy funkcji is_geometric() bez podanego ilorazu"""
    ...
    def test_zeros(self):
        """Ciąg geometryczny nie zawiera zer"""
        for seq in [[1, 0, 0], [0, 0, 0], [0, 1, 2]]:
            with self.subTest(seq=seq):
                self.assertFalse(is_geometric(seq))


class TestIsGeometricWithQ(unittest.TestCase):
    """Testy funkcji is_geometric() z podanym ilorazem q"""

    def test_empty_with_q(self):
        """Test dla pustej sekwencji z podanym ilorazem"""
        self.assertTrue(is_geometric([], q=2))

    def test_single_with_q(self):
        """Test dla jednoelementowej sekwencji z podanym ilorazem"""
        self.assertTrue(is_geometric([5], q=3))

    def test_positive(self):
        """Test dla sekwencji geometrycznych z prawidłowym ilorazem"""
        self.assertTrue(is_geometric([1, 2, 4], q=2))
        self.assertTrue(is_geometric([1, -2, 4, -8], q=-2))
        self.assertTrue(is_geometric([3, 9, 27, 81], q=3))

    def test_negative(self):
        """Test dla sekwencji geometrycznych z nieprawidłowym ilorazem"""
        self.assertFalse(is_geometric([1, 2, 4], q=3))
        self.assertFalse(is_geometric([1, -2, 4], q=2))
        self.assertFalse(is_geometric([3, 9, 27, 81], q=2))

    def test_non_geometric_with_q(self):
        """Test dla sekwencji niegeometrycznych z podanym ilorazem"""
        for seq in NON_GEOMETRIC_SEQS:
            with self.subTest(seq=seq):
                self.assertFalse(is_geometric(seq, q=2))

    def test_zeros_with_q(self):
        """Ciąg geometryczny nie zawiera zer, nawet z podanym ilorazem"""
        for seq in [[1, 0, 0], [0, 0, 0], [0, 1, 2]]:
            with self.subTest(seq=seq):
                self.assertFalse(is_geometric(seq, q=2))
